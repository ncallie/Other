...
public class Program {
    public static void main(String[] args) throws IOException {
        ...
        for (int test = 0; test < countTests; ++test) {
            ...
            List<Integer[]> matrix = new ArrayList<>(y);
            for (int i = 0; i < y; ++i) {
                String[] s1 = reader.readLine().split(" ");
                Integer [] line = new Integer[x];
                for (int j = 0; j < x; ++j) {
                    line[j] = Integer.parseInt(s1[j]);
                }
                matrix.add(line);
            }
            int countClick = Integer.parseInt(reader.readLine());
            Integer [] clicks = new Integer[countClick];
            String[] s1 = reader.readLine().split(" ");
            for (int i = 0; i < countClick; ++i) {
                clicks[i] = Integer.parseInt(s1[i]);
            }
            for (int i = 0; i < countClick; ++i) {
                int column = clicks[i] - 1;
                Collections.sort(matrix, Comparator.comparingInt(o -> o[column]));
            }
...
// B. Электронная таблица (10 баллов)
